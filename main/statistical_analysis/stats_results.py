...
def get_roc_curve(file_list):
    ...
    ## Conventional
    fpr_base, tpr_interp, thresholds_interp = calc_interp_curve(dataframe=dataframe, y="Asthma", score="MBPT_result")
    ...
    ## SVM
    fpr_base, tpr_interp, thresholds_interp = calc_interp_curve(dataframe=dataframe, y="Asthma", score="pred_proba")
    ...
    result = pd.concat([conventional_result, svm_result, lr_result, ann_result, xgb_result, rf_result], ignore_index=True)
...
def get_prc_curve(file_list):
    ...
    ## Conventional
    recall_base, precision_interp = calc_interp_prc(dataframe=dataframe, y="Asthma", score="MBPT_result")
    ...
    ## SVM
    recall_base, precision_interp = calc_interp_prc(dataframe=dataframe, y="Asthma", score="pred_proba")
...
upper = df_orig.groupby('model')['auroc'].mean() + (df_orig.groupby('model')['auroc'].std() * 1.96 / 10)
lower = df_orig.groupby('model')['auroc'].mean() - (df_orig.groupby('model')['auroc'].std() * 1.96 / 10)

print(lower)
# %%
df_orig.groupby('model')['auprc'].mean()

upper = df_orig.groupby('model')['auprc'].mean() + (df_orig.groupby('model')['auprc'].std() * 1.96 / 10)
lower = df_orig.groupby('model')['auprc'].mean() - (df_orig.groupby('model')['auprc'].std() * 1.96 / 10)
